...
export function netCost(u) {
  ...
  if (isService) {
    return {
      kind: 'service',
      label: 'Académie militaire — scolarité couverte',
      note: 'Pas de frais de scolarité, mais engagement de service militaire après le diplôme. Cas très particulier.',
      range: '≈ $0/an (scolarité)',
    }
  }
  if (u.division === 'D3') {
    return {
      kind: 'd3',
      label: 'Pas de bourse sportive (règle D3)',
      note: 'La D3 n’attribue aucune bourse sportive. Mais aides au mérite et au besoin possibles selon ton dossier scolaire.',
      range: `${k(u.costUSD)}/an (avant aide)`,
    }
  }
  if (isIvy) {
    return {
      kind: 'ivy',
      label: 'Pas de bourse sportive (Ivy League)',
      note: 'Les facs Ivy n’offrent pas de bourse sportive : uniquement de l’aide financière selon les revenus familiaux (need-based).',
      range: `${k(u.costUSD)}/an (avant aide)`,
    }
  }
  ...
  return {
    kind: 'athletic',
    label: 'Bourse sportive possible (natation = sport « equivalency »)',
    note: 'L’équipe partage ~9,9 bourses (D1) / 8,1 (D2) sur tout l’effectif → le plus souvent partielles. Cumulables avec une aide au mérite.',
    range: `≈ ${k(low)}–${k(high)}/an net selon la bourse`,
  }
}
