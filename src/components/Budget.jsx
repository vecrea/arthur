...
export default function Budget({ schools, favorites }) {
  ...
  const inputCls = 'w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-pool-500 focus:ring-2 focus:ring-pool-500/20'
    ...
    <div className="space-y-5">
      <div className="rounded-3xl bg-white p-5 shadow-card ring-1 ring-slate-900/5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('Coût annoncé / an ($, intl)', 'Sticker cost / yr ($, intl)')}</span>
            <input type="number" value={isManual ? manualCost : costUSD} onChange={(e) => setManualCost(e.target.value)} disabled={!isManual} className={inputCls + ' mt-1 ' + (isManual ? '' : 'bg-slate-50 text-slate-500')} />
          </label>
        </div>

        {info && (
          <p className="mt-2 text-xs text-slate-500">
            <span className="font-semibold text-slate-600">{info.label}.</span> {info.note}
          </p>
        )}
      </div>
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between text-sm">
              <span className="font-medium text-slate-600">{t('Bourse sportive', 'Athletic scholarship')}</span>
              <span className="font-display font-extrabold text-pool-600">{scholar}%</span>
            </div>
            ...
            <p className="text-[11px] text-slate-400">{t('La natation = bourses souvent partielles. D3/Ivy = 0 (aides au mérite à part).', 'Swimming = often partial scholarships. D3/Ivy = 0 (merit aid is separate).')}</p>
          </div>

          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <label className="block">
              <span className="text-xs font-medium text-slate-500">{t('Taux $→€', 'Rate $→€')}</span>
              <input type="number" step="0.01" value={rate} onChange={(e) => setRate(Number(e.target.value))} className={inputCls + ' mt-1'} />
            </label>
            <label className="block">
              <span className="text-xs font-medium text-slate-500">{t('Aller-retours / an', 'Round trips / yr')}</span>
              <input type="number" min="0" value={roundTrips} onChange={(e) => setRoundTrips(e.target.value)} className={inputCls + ' mt-1'} />
            </label>
            <label className="block">
              <span className="text-xs font-medium text-slate-500">{t('Prix A/R (€)', 'Round trip (€)')}</span>
              <input type="number" min="0" value={flightPrice} onChange={(e) => setFlightPrice(e.target.value)} className={inputCls + ' mt-1'} />
            </label>
            <label className="block">
              <span className="text-xs font-medium text-slate-500">{t('Années', 'Years')}</span>
              <input type="number" min="1" max="5" value={years} onChange={(e) => setYears(e.target.value)} className={inputCls + ' mt-1'} />
            </label>
          </div>
          <label className="block sm:max-w-xs">
            <span className="text-xs font-medium text-slate-500">{t('Visa + SEVIS (1× la 1re année, €)', 'Visa + SEVIS (one-time, year 1, €)')}</span>
            <input type="number" min="0" value={visaFees} onChange={(e) => setVisaFees(e.target.value)} className={inputCls + ' mt-1'} />
          </label>
        </div>
      </div>
  ...
  return (
    <tr className="border-t border-slate-100">
      <td className="px-5 py-2.5">
        <div className={(bold ? 'font-extrabold ' : 'font-medium ') + 'text-navy-900'}>{label}</div>
        {sub && <div className="text-[11px] text-slate-400">{sub}</div>}
      </td>
      <td className={'px-5 py-2.5 text-right tabular-nums ' + (accent ? 'font-display text-lg font-black text-emerald-700' : bold ? 'font-extrabold text-navy-900' : 'text-slate-600')}>{val}</td>
    </tr>
  )
}
