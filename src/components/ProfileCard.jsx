...
function Info({ label, value }) {
  return (
    <div className="surface border border-hair rounded-lg p-3">
      <div className="text-[11px] font-semibold uppercase tracking-wide text-tertiary">{label}</div>
      <div className="mt-0.5 font-semibold text-heading">{value}</div>
    </div>
  )
}
...
export default function ProfileCard({ profile }) {
  ...
  return (
    <div className="panel overflow-hidden">
      {/* Bandeau sombre — identité + niveau */}
      ...
      <div className="border-t border-hair">
        <div className="flex items-center justify-between border-b border-hair px-4 py-3">
          <h3 className="font-display text-lg font-extrabold text-heading">{t('Tes temps', 'Your times')}</h3>
          <span className="text-xs text-tertiary">{t('grand bassin → yards (US)', 'long course → yards (US)')}</span>
        </div>
              ...
              {EVENTS.filter((e) => byEvent[e.key]).map((e) => {
                ...
                return (
                  <tr key={e.key} className="border-t border-hair">
                    <td className="px-4 py-2 font-semibold text-heading">{t(e.label, e.labelEn)}</td>
                    <td className="px-4 py-2 tabular-nums text-primary">{formatTime(d.lcm)}</td>
                    <td className="px-4 py-2 tabular-nums text-secondary">{formatTime(d.scy)}</td>
        ...
        <p className="border-t border-hair px-4 py-3 text-xs text-tertiary">
          {t(
      ...
      <div className="border-t border-hair p-5">
        <h3 className="font-display text-lg font-extrabold text-heading">{t('Ta Road to D1', 'Your Road to D1')}</h3>
        <p className="mt-1 text-sm text-primary">
          {t(
            <>
              ...
              <span className="font-semibold text-emerald-600 dark:text-emerald-400">Réaliste</span>, <span className="font-semibold text-accent">Objectif</span> et{' '}
              <span className="font-semibold text-spark-600 dark:text-spark-400">Ambitieux</span>.
            </>,
            <>
              ...
              <span className="font-semibold text-emerald-600 dark:text-emerald-400">Safety</span>, <span className="font-semibold text-accent">Target</span> and{' '}
              <span className="font-semibold text-spark-600 dark:text-spark-400">Reach</span>.
            </>,
          )}
        </p>
      ...
      <div className="border-t border-hair p-5">
        <h3 className="font-display text-lg font-extrabold text-heading">{t('Sauvegarde & synchro', 'Backup & sync')}</h3>
        <p className="mt-1 text-sm text-primary">
          {t(
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => fileRef.current?.click()} className="rounded-full surface-2 border border-hair px-4 py-2 text-sm font-semibold text-primary transition hover:surface-3">
          ...
          {backupMsg && <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-400">{backupMsg}</span>}
        </div>
