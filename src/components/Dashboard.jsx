...
export default function Dashboard({ profile, matches, favCount, setTab }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Widgets — cellules séparées par des filets, pas des cartes flottantes */}
      <div className="grid gap-px border-t border-hair sm:grid-cols-2 lg:grid-cols-3" style={{ background: 'var(--border)' }}>
        ...
        <Tile onClick={() => setTab('steps')} title={t('Démarches', 'Steps')} cta={t('Continuer', 'Continue')}>
          <div className="flex items-end gap-2">
            <span className="font-display text-3xl font-black text-accent">{pct}%</span>
            <span className="pb-1 text-xs text-tertiary">{doneCount}/{ALL_IDS.length}</span>
          </div>
          <div className="mt-1 h-2 w-full overflow-hidden rounded-full surface-3">
        ...
        {/* Cellule de remplissage pour compléter la grille (3 colonnes → 7 tuiles + 1) */}
